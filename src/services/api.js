...
var api = {
  ...
  handleReturn: function (response, onSuccess, onError) {
    if (response.data.message != undefined) {
      ...
      console.warn('stack = ' + response.data.stack)
    }
  ...
  handleError: function (error, onError) {
    ...
    if (error.response == undefined) {
      ...
      if (error.message !== undefined)
        ...
        errorMessage = "Houve um erro ao processar esta informação";
    }
    else if (error.response.status == 401)
      errorMessage = "A sessão expirou. Faça o login novamente. "
    else if (error.response.status == 500) {
      errorMessage = "Erro ao obter as informações do servidor. "
      if (error.response.data != null && error.response.data.ExceptionMessage != null)
        errorMessage += error.response.data.ExceptionMessage
    }
    else if (error.response.data != null && error.response.data.error_description != null)
  ...
  getToken: function (username, password, onSuccess, onError) {
    ...
    var reqData = "username=" + username + '&password=' + password + "&grant_type=password";

    this.axiosApi.post('/token', reqData,
      ...
      .then(response => api.handleReturn(response, async data => {
  ...
  apiPost: async function (url, Parameters, onSuccess, onError, contentType) {
    ...
    this.axiosApi.post(url, Parameters,
      {
        headers: headers
      })
      .then(response => api.handleReturn(response, onSuccess, onError))
